...
# 使用suppressWarnings抑制参数警告
bar <- suppressWarnings(
  barplot(kk, drop=TRUE, showCategory=showNum, label_format=30, split="ONTOLOGY", color=colorSel) + 
...
bub <- suppressWarnings(
  dotplot(kk, showCategory=showNum, orderBy="GeneRatio", label_format=30, split="ONTOLOGY", color=colorSel) + 
  facet_grid(ONTOLOGY~., scale='free')
)
print(bub)
dev.off()
